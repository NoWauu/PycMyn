...
class Sequence:
    ...
    def __init__(self, seq: List[Tuple[Tuple[Callable[..., None], List[Any]], float]], loop: bool = False) -> None:
        self.fnct: List[Tuple[Callable[..., None], List[Any]]] = []
        self.times: List[float] = []
        self.is_running = False
        self.start_sqc_time = pygame.time.get_ticks()
        self.pointer = 0
        self.loop = loop
        for elm in seq:
    ...
    def update(self):
        ...
        if self.is_running:
            if self.times[self.pointer] <= pygame.time.get_ticks() - self.start_sqc_time:
                self.fnct[self.pointer][0](*self.fnct[self.pointer][1])
                self.pointer += 1
